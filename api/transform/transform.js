...
//チャートと円グラフで使用するデータ
function transformToHL() {
  // getJson関数もインポートする場合は同じファイルから
  ...
  const monthlySortedUser = sortMonthlyUser(monthlySumPriceByUser); // --- IGNORE
...
function getIndicatorR() {
  ...
  const jsonData = fs.readFileSync(filePath, "utf8");
  const indicatorR = JSON.parse(jsonData);
  return indicatorR;
}
  ...
  for (const month in monthlyPriceByUser) {
    ...
    for (const userId in monthlyPriceByUser[month]) {
      ...
      const totalPrice = orders.reduce(
        (sum, order) => sum + (order.item?.price || 0),
        0
      );
...
function sortUserDays(userDays) {
  ...
  // 各期間設定について処理
  for (const [periodType, config] of Object.entries(indicatorR)) {
    const sectionCount = {};
    ...
    for (const userId in userDays) {
      ...
      const section = getSection(days, config);
      if (section) {
        sectionCount[section]++;
      }
    }

    results[periodType] = sectionCount;
  }
